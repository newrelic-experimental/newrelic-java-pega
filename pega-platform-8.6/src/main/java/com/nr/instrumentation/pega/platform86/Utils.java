package com.nr.instrumentation.pega.platform86;
...
public class Utils {
	...
	public static void addJobRuntime(Map<String,Object> attributes, JobRunTime job) {
		if(job != null) {
			addAttribute(attributes, "JobId", job.getJobId());
			Optional<String> reqId = job.getRequestorId();
			if(reqId.isPresent()) {
				addAttribute(attributes, "RequestorId", reqId.get());
			}
			addAttribute(attributes, "JobStatus", job.getStatus());
		}
	}
